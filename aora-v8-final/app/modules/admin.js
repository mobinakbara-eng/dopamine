...
function kioskAdminPage(){
  ...
  return head("Kiosk","Lokale Check-in-Geräte verwalten.",'<button class="btn" data-a="kiosk-create-modal">Kiosk-Gerät einrichten</button>')+`<div class="admin-dashboard">${devices.map(device=>`<article class="dashboard-card" style="min-height:220px;padding:20px"><div class="caps muted">Gerät</div><h2 style="margin-top:10px">${esc(device.name)}</h2><p class="small muted" style="margin-top:8px">${device.locked?"Gesperrt":"Aktiv"} · Version ${device.activationVersion||1}</p><p class="small muted mono" style="margin-top:8px">${esc(device.id)}</p><div class="delivery-actions" style="margin-top:25px"><button class="btn outline" data-a="rotate-kiosk" data-id="${device.id}">Neuen Code</button><button class="btn ${device.locked?"":"outline"}" data-a="toggle-kiosk" data-id="${device.id}">${device.locked?"Entsperren":"Sperren"}</button></div></article>`).join("")||'<div class="empty">Noch kein Kiosk-Gerät vorhanden. Richte das erste Gerät für diesen Laden ein.</div>'}</div>`;
}
