...
(function installFinalReportPdfFormat(){
  const defaultRange=()=>{
  ...
  const entryStart=item=>clockValue(item?.start||item?.startTime||item?.start_time);
  const entryEnd=item=>clockValue(item?.end||item?.endTime||item?.end_time);
  ...
  const duration=item=>{
    if(Number.isFinite(Number(item?.durationMinutes))&&Number(item.durationMinutes)>=0&&entryEnd(item))return Number(item.durationMinutes);
  ...
  const aggregateEntries=items=>{
    ...
    const open=entries.filter(item=>!entryEnd(item)||["live","paused"].includes(String(item.status||"").toLowerCase()));
    const closed=entries.filter(item=>entryEnd(item)&&!["live","paused"].includes(String(item.status||"").toLowerCase()));
  ...
  const pendingCorrectionCount=(employeeId,periodEntries)=>{
    const entryIds=new Set(periodEntries.map(item=>String(item?.id||"")));
    return(S.state?.correctionRequests||[]).filter(item=>{
      const status=String(item?.status||"").toLowerCase(),entryId=String((item?.entryId??item?.entry_id)||"");
      return String(item?.employeeId??item?.employee_id)===String(employeeId)&&(!entryId||entryIds.has(entryId))&&!["approved","accepted","rejected","declined","cancelled"].includes(status);
    }).length;
  };
  ...
  function reportData(employee,from,to){
    ...
    const rows=enumerateDates(from,to).map(date=>{
      ...
      if(dateEntries.length){
        row={...row,...aggregateEntries(dateEntries)};
        if(leave)row.note=[row.note,`Abwesenheit parallel erfasst: ${leaveType(leave)}`].filter(Boolean).join(" · ");
      }else if(leave){
        const plannedForDay=dateShifts.reduce((sum,item)=>sum+duration(item),0);
        const credited=plannedForDay||([1,2,3,4,5].includes(dateObject(date).getUTCDay())?dailyTarget:0);
        row={...row,type:leaveType(leave),start:"–",end:"–",netMinutes:credited,note:text(leave.note||leave.reason||"Genehmigte Abwesenheit")};
      }else if(dateShifts.length){
        row={...row,type:"Fehlzeit",start:dateShifts.map(item=>text(item.start)||"–").join(" / "),end:dateShifts.map(item=>text(item.end)||"–").join(" / "),breakMinutes:dateShifts.reduce((sum,item)=>sum+Math.max(0,Number(item.breakMinutes)||0),0),note:dateShifts.length>1?`Keine Zeitbuchung · ${dateShifts.length} geplante Schichten`:"Keine Zeitbuchung"};
    ...
    const closed=entries.filter(item=>displayedWorkDates.has(entryDate(item))&&entryEnd(item)&&!["live","paused"].includes(String(item.status||"").toLowerCase()));
    const openEntries=entries.filter(item=>!entryEnd(item)||["live","paused"].includes(String(item.status||"").toLowerCase())).length;
    const missingDays=rows.filter(row=>row.type==="Fehlzeit").length;
    const pendingCorrections=pendingCorrectionCount(employeeId,entries);
    ...
    return{
      ...
      entryCount:entries.length,openEntries,missingDays,pendingCorrections,
      status:openEntries||missingDays||pendingCorrections?"Prüfung offen":"Vollständig"
    };
  }
  ...
  const typeClass=value=>String(value||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9-]/g,"-");
  const reportRows=rows=>rows.map(row=>`<tr class="${["Offen","Fehlzeit"].includes(row.type)?"attention":""}"><td>${esc(displayDate(row.date))}</td><td>${esc(row.day)}</td><td>${esc(row.start)}</td><td>${esc(row.end)}</td><td>${esc(formatMinutes(row.breakMinutes))}</td><td><strong>${esc(formatMinutes(row.netMinutes))}</strong></td><td><span class="aora-report-type ${typeClass(row.type)}">${esc(row.type)}</span></td><td>${esc(row.note||"–")}</td></tr>`).join("");

  function reportSheet(data,from,to,{printOnly=false}={}){
    const employee=data.employee,meta=employeeMeta(employee),location=loc(employee.locationId||employee.primaryLocationId),company=text(S.state?.company?.name)||"Arbeitgeber";
    const statusClass=data.status==="Vollständig"?"confirmed":"open";
    return`<article class="aora-report-sheet aora-report-final ${printOnly?"print-only":""}">
      <header class="aora-report-document-head"><div class="aora-report-brand aora-report-employer"><strong>${esc(company)}</strong><span>${esc(location?.name||"Standort")}</span></div><div class="aora-report-document-title"><span>Arbeitszeitnachweis</span><strong>Stundenübersicht</strong></div><div class="aora-report-created"><span>Erstellt am</span><strong>${esc(new Intl.DateTimeFormat("de-DE",{dateStyle:"medium",timeStyle:"short"}).format(new Date()))}</strong></div></header>
      ...
      <section class="aora-report-daily"><div class="aora-report-section-head"><div><span>Tägliche Übersicht</span><strong>${data.rows.length} Kalendertage · alle Buchungen vollständig zusammengeführt</strong></div><div class="aora-report-status ${statusClass}">${data.status==="Vollständig"?I.check:I.clock}${esc(data.status)}</div></div><div class="aora-report-table-wrap"><table class="aora-report-table"><thead><tr><th>Datum</th><th>Tag</th><th>Start</th><th>Ende</th><th>Pause</th><th>Netto</th><th>Typ</th><th>Bemerkung</th></tr></thead><tbody>${reportRows(data.rows)}</tbody><tfoot><tr><td colspan="5">Gesamtsumme</td><td>${esc(formatMinutes(data.total))}</td><td colspan="2">Differenz ${esc(formatMinutes(data.difference,{signed:true}))}</td></tr></tfoot></table></div></section>
      <section class="aora-report-footer-stats four"><div><span>Arbeitstage</span><strong>${data.workDays}</strong></div><div><span>Offene Buchungen</span><strong>${data.openEntries}</strong></div><div><span>Fehlende Buchungen</span><strong>${data.missingDays}</strong></div><div><span>Offene Korrekturen</span><strong>${data.pendingCorrections}</strong></div></section>
      <footer class="aora-report-signatures aora-report-signatures-final"><div><span>Ort, Datum (optional)</span><b></b></div><div><span>Unterschrift Mitarbeiter/in (optional)</span><b></b></div><div><span>Unterschrift Arbeitgeber/Vorgesetzte/r (optional)</span><b></b></div></footer>
      <p class="aora-report-legal-note">Elektronisch erstellter Arbeitszeitnachweis. Die Unterschriftsfelder sind optional; die Verantwortung für die Richtigkeit der Arbeitszeitaufzeichnung bleibt beim Arbeitgeber.</p>
      <div class="aora-report-page-footer"><span>${esc(company)}</span><span>${esc(location?.name||"Standort")} · ${esc(rangeLabel(from,to))}</span></div>
    </article>`;
  }
  ...
  reportsPage=function(ownerMode=false){
    ...
    const intro=`<div class="compliance-alert"><strong>PDF-Arbeitszeitnachweis</strong><span>Alle Buchungen eines Tages werden zusammengeführt. Die PDF-Erstellung ist unabhängig von einer Unterschrift; Freigaben können zusätzlich als interner Bestätigungsprozess genutzt werden.</span></div>`;
    ...
    return`${head("Arbeitszeitberichte","Arbeitszeiten vollständig prüfen und als druckfertiges A4-PDF ausgeben.")}${intro}${filters(ownerMode,employees,filtersValue)}<div class="aora-report-preview-shell"><div class="aora-report-preview-label"><span>PDF-Vorschau</span><small>A4 · Unterschriften optional</small></div>${preview}</div>`;
  };

  function toastMessage(message){
    if(typeof toast==="function"){toast(message);return}
    const root=document.getElementById("toast-root");if(!root)return;
    const item=document.createElement("div");item.className="toast";item.textContent=message;root.appendChild(item);setTimeout(()=>item.remove(),3000);
  }
  window.aoraReportBuildPrintBundle=function(all=false){
    ...
    if(!targets.length){toastMessage("Keine Mitarbeiterdaten für den gewählten Zeitraum vorhanden.");return}
    ...
    const oldTitle=document.title;document.title=selected&&!all?`Arbeitszeitnachweis – ${selected.name} – ${rangeLabel(filtersValue.from,filtersValue.to)}`:`Arbeitszeitnachweise – ${rangeLabel(filtersValue.from,filtersValue.to)}`;
    let cleaned=false;
    let fallback=0;
    const cleanup=()=>{
      if(cleaned)return;cleaned=true;clearTimeout(fallback);
      bundle.remove();document.body.classList.remove("aora-report-printing");document.title=oldTitle;
      window.removeEventListener("afterprint",cleanup);
    };
    window.addEventListener("afterprint",cleanup,{once:true});
    fallback=setTimeout(cleanup,30000);
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      try{window.print()}catch(error){cleanup();console.error("PDF print failed",error);toastMessage("PDF konnte nicht geöffnet werden.")}
    }));
  };
})();
