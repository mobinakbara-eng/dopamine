...
export async function listTransferSuggestions(ctx:InventoryContext,body:any,requestId:string){
  ...
  const[
    ...
    db.from("inventory_items")
      .select("id,name,sku,base_uom,category")
      .eq("organization_id",ctx.organizationId)
      .in("id",itemIds),
    db.from("locations")
      .select("id,name,city")
      ...
      .eq("active",true)
      .is("deleted_at",null),
    db.from("inventory_balances")
      .select("location_id,item_id,on_hand,reserved")
      .eq("organization_id",ctx.organizationId)
      .in("location_id",sourceLocationIds)
      .in("item_id",itemIds),
    db.from("inventory_item_locations")
      .select("location_id,item_id,reorder_point,par_level")
      .eq("organization_id",ctx.organizationId)
      .eq("active",true)
      .in("location_id",sourceLocationIds)
      .in("item_id",itemIds),
    db.from("inventory_transfers")
      .select("id")
      .eq("organization_id",ctx.organizationId)
  ...
  const{data:draftLines,error:dle}=draftIds.length
    ?await db.from("inventory_transfer_lines")
      .select("item_id,requested_quantity")
      .eq("organization_id",ctx.organizationId)
      .in("transfer_id",draftIds)
      .in("item_id",itemIds)
    :{data:[],error:null};
  ...
  for(const line of draftLines||[]){
    ...
    draftIncoming.set(key,(draftIncoming.get(key)||0)+Number(line.requested_quantity||0));
  }

  const suggestions:any[]=[];
  for(const state of states){
    const itemId=String(state.item_id),item:any=itemMap.get(itemId);
    const alreadyRequested=Math.max(0,draftIncoming.get(itemId)||0);
    const rawNeed=Math.max(0,Number(state.suggested_base_quantity||0));
    const need=Math.max(0,rawNeed-alreadyRequested);
    if(need<=0)continue;

    const candidates:any[]=[];
    for(const balance of balances||[]){
      if(String(balance.item_id)!==itemId)continue;
      const sourceLocationId=String(balance.location_id),location:any=locationMap.get(sourceLocationId);
      if(!location)continue;
      const policy:any=policyMap.get(`${sourceLocationId}:${itemId}`);
      if(!policy)continue;

      // Only move stock physically present now, and never push the source below its
      // own PAR/reorder safety floor. Incoming supplier stock is intentionally not
      // counted as transferable stock.
      const sourceAvailable=Math.max(0,Number(balance.on_hand||0)-Number(balance.reserved||0));
      const sourceFloor=Math.max(Number(policy.reorder_point||0),Number(policy.par_level??0));
      const surplus=Math.max(0,sourceAvailable-sourceFloor);
      if(surplus<=0)continue;

      candidates.push({
        sourceLocationId,
        sourceLocationName:String(location.name||sourceLocationId),
        sourceCity:String(location.city||""),
        sourceOnHand:Number(balance.on_hand||0),
        sourceReserved:Number(balance.reserved||0),
        sourceSafetyFloor:sourceFloor,
        transferableQuantity:surplus
      });
    }
    candidates.sort((a,b)=>b.transferableQuantity-a.transferableQuantity||a.sourceLocationName.localeCompare(b.sourceLocationName,"de"));
    if(!candidates.length)continue;

    const totalTransferable=candidates.reduce((sum,x)=>sum+Number(x.transferableQuantity||0),0);
    const best=candidates[0],recommendedQuantity=Math.min(need,Number(best.transferableQuantity||0));
    if(recommendedQuantity<=0)continue;

    suggestions.push({
      itemId,
      episodeId:state.episode_id||null,
      item:item||null,
      needQuantity:need,
      rawNeedQuantity:rawNeed,
      alreadyRequestedQuantity:alreadyRequested,
      totalTransferableQuantity:totalTransferable,
      canFullyCover:totalTransferable>=need,
      recommendedSourceLocationId:best.sourceLocationId,
      recommendedSourceLocationName:best.sourceLocationName,
      recommendedQuantity,
      remainingAfterRecommendation:Math.max(0,need-recommendedQuantity),
      candidates,
...
export async function listPurchaseOrders(ctx:InventoryContext,body:any,requestId:string){
  ...
  const{data:orders,error}=await db.from("inventory_purchase_orders")
    .select("id,supplier_id,status,expected_on,note,version,order_number,delivery_channel,provider_message_id,provider_status,last_error,created_at,submitted_at,sent_at,received_at")
    .eq("organization_id",ctx.organizationId)
  ...
  if(!orderIds.length)return{locationId,orders:[]};
  ...
  return{locationId,orders:(orders||[]).map((o:any)=>({...o,supplier:sm.get(String(o.supplier_id))||null,lines:(lines||[]).filter((l:any)=>String(l.purchase_order_id)===String(o.id)).map((l:any)=>({...l,orderedQuantity:Number(l.ordered_quantity),receivedQuantity:Number(l.received_quantity),orderedPackQuantity:l.ordered_pack_quantity==null?null:Number(l.ordered_pack_quantity),unitCost:l.unit_cost==null?null:Number(l.unit_cost),item:im.get(String(l.item_id))||null,pack:pm.get(String(l.pack_unit_id))||null}))}))};
}

export async function listPackUnits(ctx:InventoryContext,body:any,requestId:string){
  const locationId=String(body.locationId||"");
  await requirePermission(ctx,locationId,"view",requestId);
  const{data,error}=await db.from("inventory_pack_units")
    .select("id,item_id,code,label,base_quantity,is_stock_unit,is_order_unit,active")
    .eq("organization_id",ctx.organizationId)
    .eq("item_id",String(body.itemId||""))
    .eq("active",true)
    .order("base_quantity");
  if(error)dbFail(error,"pack_units",requestId);
  return{packUnits:(data||[]).map((r:any)=>({...r,baseQuantity:Number(r.base_quantity)}))};
}
...
export async function listPrintJobs(ctx:InventoryContext,body:any,requestId:string){
  ...
  const{data,error}=await db.from("inventory_label_print_jobs")
    .select("id,receipt_id,purchase_order_id,item_id,pack_unit_id,label_count,status,generation,prepared_at,printed_at,created_at")
    .eq("organization_id",ctx.organizationId)
    .eq("location_id",locationId)
  ...
  const{data:items,error:ie}=ids.length?await db.from("inventory_items").select("id,name,sku,base_uom").eq("organization_id",ctx.organizationId).in("id",ids):{data:[],error:null};
  if(ie)dbFail(ie,"print_job_items",requestId);
  const im=new Map((items||[]).map((r:any)=>[String(r.id),r]));
  return{locationId,jobs:(data||[]).map((r:any)=>({...r,labelCount:Number(r.label_count),generation:Number(r.generation),item:im.get(String(r.item_id))||null}))};
}
...
export async function listReplenishment(ctx:InventoryContext,body:any,requestId:string){
  const locationId=String(body.locationId||"");
  await requirePermission(ctx,locationId,"procurement",requestId);
  const{data,error}=await db.from("inventory_replenishment_state")
    .select("item_id,below_threshold,episode_id,opened_at,suggested_base_quantity,updated_at")
    .eq("organization_id",ctx.organizationId)
    .eq("location_id",locationId)
    .eq("below_threshold",true)
    .order("opened_at");
  if(error)dbFail(error,"replenishment",requestId);
  return{suggestions:(data||[]).map((r:any)=>({...r,suggestedQuantity:Number(r.suggested_base_quantity)}))};
}
