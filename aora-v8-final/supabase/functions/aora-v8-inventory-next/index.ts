...
Deno.serve(async request=>{
  ...
  try{
    const raw=await request.text();if(new TextEncoder().encode(raw).byteLength>MAX_BODY_BYTES)fail(413,"request_too_large","Die Anfrage ist zu groß.");
    let body:any;try{body=JSON.parse(raw)}catch{fail(400,"invalid_json","Ungültige Anfrage.")}
    const action=String(body.action||"");
    if(action==="health")return json({ok:true,service:"aora-v8-inventory-next",version:2,emailProviderConfigured:Boolean((Deno.env.get("RESEND_API_KEY")||Deno.env.get("AORA_ORDER_EMAIL_API_KEY"))&&Deno.env.get("AORA_ORDER_FROM_EMAIL")),serverTime:new Date().toISOString()},200,origin,requestId);
    const ctx=await sessionContext(String(body.token||""),requestId);
  ...
  }catch(error){const e=error instanceof ApiError?error:new ApiError(500,"internal_error","Die Aktion konnte nicht abgeschlossen werden.");console.warn("aora-inventory-rejected",{requestId,status:e.status,code:e.code,message:e.message});return json({error:e.message,code:e.code,requestId},e.status,origin,requestId)}
});
