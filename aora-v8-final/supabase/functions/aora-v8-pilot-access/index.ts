...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "passwordLogin") {
      ...
      if (!["owner", "manager", "employee"].includes(accessRole)) return reply({ error: "E-Mail oder Passwort ist nicht korrekt." }, 401, origin);
      const rate = await consumeRateLimit(request, slug, "password", `${accessRole}:${email}`);
      if (!rate.allowed) return reply({ error: "Zu viele Anmeldeversuche.", retryAfter: rate.retryAfter }, 429, origin);
      const account = accountRecord(context.state, email);
      ...
      let valid = false;
      if (
        account && account.accessRole === accessRole && account.record.status === "active" &&
