...
function requireWorkflowRef(payload: any, eventName: string) {
  const workflowRef = String(payload.workflow_ref || "");
  if (!workflowRef.startsWith(WORKFLOW_PATH)) fail("workflow_not_allowed", 403);
  const sourceRef = workflowRef.slice(WORKFLOW_PATH.length);
  if (eventName === "pull_request") {
    if (!/^refs\/pull\/\d+\/merge$/.test(sourceRef) && sourceRef !== "refs/heads/main") fail("workflow_ref_not_allowed", 403);
  } else if (sourceRef !== "refs/heads/main") fail("workflow_ref_not_allowed", 403);
  return workflowRef;
}
async function claims(request: Request) {
  const payload = await verifyOidc(bearer(request));
  if (payload.repository !== REPOSITORY || String(payload.repository_id || "") !== REPOSITORY_ID) fail("repository_not_allowed", 403);
  const eventName = String(payload.event_name || "");
  requireActor(payload, eventName);
  const workflowRef = requireWorkflowRef(payload, eventName);
  ...
  if (eventName === "pull_request") {
    ...
    if (String(payload.ref || "") !== "refs/heads/main" || String(payload.ref_type || "") !== "branch") fail("dispatch_ref_not_allowed", 403);
...
async function rpc(name: string, body: any) {
  ...
  if (!result.ok) fail(String(result.body?.message || result.body?.code || "rpc_failed").slice(0, 500), result.status);
  return result.body;
}
function password() {
  return "Aora-" + randomHex(12) + "-9aZ";
}
function email(role: string, runId: string, attempt: string, suffix: string) {
  return (`aora-timesheet-${role}-${runId}-${attempt}-${suffix}@example.com`).toLowerCase();
}
function berlinDate(offsetDays = 0) {
  return new Intl.DateTimeFormat("sv-SE", { timeZone: "Europe/Berlin" }).format(new Date(Date.now() + offsetDays * 86400000));
}
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const identity = await claims(request);
    ...
    console.error("Aora timesheet CI bootstrap error", String(error?.message || error));
