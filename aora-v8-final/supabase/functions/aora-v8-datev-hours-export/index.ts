...
function monthRange(period: string) {
  ...
  return {
    ...
    datev: `01/${String(month).padStart(2, "0")}/${year}`,
  };
}
...
function employeeLocation(employee: any) {
  return String(employee?.locationId || employee?.primaryLocationId || "");
}
function entryEmployeeId(entry: any) {
  return String(entry?.employeeId ?? entry?.employee_id ?? "");
}
function entryLocationId(entry: any) {
  return String(entry?.locationId ?? entry?.location_id ?? "");
}
function entryDate(entry: any) {
  return String(entry?.date || entry?.startTime || entry?.start_time || "").slice(0, 10);
}
...
function assertAscii(value: string) {
  for (const character of value) if (character.charCodeAt(0) > 127) fail("DATEV-Datei enthält nicht unterstützte Zeichen.", 422, "datev_ascii_non_ascii");
  return value;
}
...
async function context(token: string) {
  ...
  const admin = session.role === "admin"
    ? state.admins.find((item: any) => item.id === session.subject_id && item.active !== false && item.status !== "revoked" && item.status !== "pending")
    : null;
  ...
  return { session, organization, state, admin, accessRole, locationIds };
}
function requireOwner(ctx: any) {
  if (ctx.accessRole !== "owner") fail("Nur der Inhaber darf DATEV-Zuordnungen ändern.", 403, "owner_required");
}
...
async function mappingRows(ctx: any, employeeIds: string[]) {
  ...
  const { data, error } = await service.from("datev_hours_employee_mappings")
    .select("employee_id,personnel_number,updated_at")
...
async function settingsRow(ctx: any) {
  const { data, error } = await service.from("datev_hours_export_settings")
    .select("target_system,berater_number,mandant_number,regular_wage_type,updated_at")
    .eq("organization_id", ctx.organization.id)
    .maybeSingle();
  if (error) throw error;
  return data;
...
async function status(ctx: any, body: any) {
  ...
  const [{ scoped, summary }, settings] = await Promise.all([
    Promise.resolve(hoursSummary(ctx, period)),
    settingsRow(ctx),
  ]);
  ...
  const employees = scoped.employees.map((employee: any) => {
    ...
    const mapping = mappingMap.get(String(employee.id));
    return {
      id: String(employee.id),
      name: clean(employee.name || "Mitarbeiter/in"),
      locationId: employeeLocation(employee),
      active: employee.active !== false,
      personnelNumber: mapping?.personnel_number || null,
      minutes: totals.minutes,
      entries: totals.entries,
      openEntries: totals.openEntries,
      included: totals.minutes > 0 || totals.openEntries > 0,
    };
  });
  ...
  return {
    ...
    employees,
    totals: {
      employees: included.length,
      minutes: included.reduce((sum: number, employee: any) => sum + Number(employee.minutes || 0), 0),
      openEntries: included.reduce((sum: number, employee: any) => sum + Number(employee.openEntries || 0), 0),
      missingPersonnelNumbers: included.filter((employee: any) => !employee.personnelNumber).length,
    },
    canConfigure: ctx.accessRole === "owner",
  };
}
async function saveConfig(ctx: any, body: any) {
  ...
  const { error: settingError } = await service.from("datev_hours_export_settings").upsert({
    organization_id: ctx.organization.id,
    target_system: "datev_lodas",
    berater_number: beraterNumber,
    mandant_number: mandantNumber,
    regular_wage_type: regularWageType,
  ...
  for (const mapping of mappings) {
    ...
    const { error } = await service.from("datev_hours_employee_mappings").upsert({
      ...
      personnel_number: mapping.personnelNumber,
      updated_by: String(ctx.session.subject_id),
      updated_at: now,
...
async function createExport(ctx: any, body: any, origin: string | null) {
  ...
  const { scoped, summary } = hoursSummary(ctx, period);
  const includedEmployees = scoped.employees.filter((employee: any) => {
    const totals = summary.get(String(employee.id));
    return totals && (totals.minutes > 0 || totals.openEntries > 0);
  });
  ...
  const rows = includedEmployees
    .map((employee: any) => ({
      employee,
      personnel: mappingMap.get(String(employee.id))!,
      minutes: Number(summary.get(String(employee.id))?.minutes || 0),
    }))
    .filter(row => row.minutes > 0)
    .sort((a, b) => a.personnel.localeCompare(b.personnel, "de", { numeric: true }))
    .map(row => `3;${scoped.range.datev};${decimalComma(row.minutes / 60)};01;${settings.regular_wage_type};${row.personnel};`);
  ...
  const { error: logError } = await service.from("datev_hours_export_runs").insert({
    organization_id: ctx.organization.id,
    period,
    target_system: "datev_lodas",
    ...
    metadata: {
      kind: "regular_hours_only",
      bookingKey: "01",
      wageType: settings.regular_wage_type,
      scope: ctx.accessRole,
      managerLocationIds: ctx.accessRole === "manager" ? ctx.locationIds : [],
    },
  });
  ...
  return new Response(bytes, {
    ...
    headers: {
      ...
      "X-Aora-Export-Checksum": checksum,
      "X-Aora-Export-Period": period,
    },
  });
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return json({
      ok: false,
      error: error?.code || "datev_hours_export_failed",
      message: clean(error?.message) || "DATEV-Stundenexport fehlgeschlagen.",
