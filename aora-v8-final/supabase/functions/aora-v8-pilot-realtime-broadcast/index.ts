...
function allowed(origin:string|null){if(!origin)return true;if(EXACT.has(origin))return true;try{const url=new globalThis.URL(origin);return ["localhost","127.0.0.1"].includes(url.hostname)||(url.protocol==="https:"&&url.hostname.endsWith(TEAM_SUFFIX))}catch{return false}}
