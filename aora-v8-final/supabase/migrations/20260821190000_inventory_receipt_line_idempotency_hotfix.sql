...
-- A single delivery may legitimately contain the same item on multiple input lines
-- (for example different lots or pack units). The delivery itself is idempotent by
-- p_idempotency_key, while each good receipt line must get its own movement key.
...
begin
  ...
  if jsonb_typeof(p_lines)<>'array' or jsonb_array_length(p_lines)=0 or jsonb_array_length(p_lines)>100 then raise exception using errcode='22023',message='inventory_receipt_lines_invalid'; end if;
  ...
  insert into public.inventory_goods_receipts(organization_id,id,location_id,purchase_order_id,status,idempotency_key,received_by)
  values(p_organization_id,v_receipt_id,p_location_id,p_purchase_order_id,'received',p_idempotency_key,p_actor_id);

  for v_input in
    select * from jsonb_to_recordset(p_lines) as x(
      item_id uuid,pack_unit_id uuid,good_pack_count integer,damaged_pack_count integer,
      missing_pack_count integer,note text,lot_code text,expires_on date
    )
  loop
    if v_input.item_id is null or v_input.pack_unit_id is null then raise exception using errcode='22023',message='inventory_receipt_line_invalid'; end if;
    if coalesce(v_input.good_pack_count,0)<0 or coalesce(v_input.damaged_pack_count,0)<0 or coalesce(v_input.missing_pack_count,0)<0
       or coalesce(v_input.good_pack_count,0)>10000 or coalesce(v_input.damaged_pack_count,0)>10000 or coalesce(v_input.missing_pack_count,0)>10000
      then raise exception using errcode='22023',message='inventory_receipt_quantity_invalid'; end if;
    if coalesce(v_input.good_pack_count,0)+coalesce(v_input.damaged_pack_count,0)+coalesce(v_input.missing_pack_count,0)<=0
      then raise exception using errcode='22023',message='inventory_receipt_quantity_invalid'; end if;
    ...
    if v_observed>v_open+0.000001 then raise exception using errcode='22023',message='inventory_purchase_order_quantity_exceeded'; end if;
    ...
    if v_good>0 and v_item.expiry_tracking and v_expires_on is null then raise exception using errcode='22023',message='inventory_expiry_required'; end if;
    if v_expires_on is not null and v_expires_on<current_date then raise exception using errcode='22023',message='inventory_expiry_in_past'; end if;
    ...
    if v_good>0 then
      ...
      insert into public.inventory_goods_receipt_lines(
        ...
        p_organization_id,v_receipt_id,v_line_no,v_input.item_id,v_input.pack_unit_id,v_input.good_pack_count,v_good,v_lot_code,v_expires_on
      );
      update public.inventory_purchase_order_lines
      set received_quantity=received_quantity+v_good
      where organization_id=p_organization_id and purchase_order_id=p_purchase_order_id and item_id=v_input.item_id;

      -- Unique inside this delivery even when the same item appears on more than one line.
      v_move:=public.aora_inventory_apply_movement(
        ...
        p_actor_id,p_actor_role,p_idempotency_key||':movement:'||v_line_no::text||':'||v_input.item_id::text
      );
      ...
      if v_pack.is_stock_unit then
        ...
        insert into public.inventory_label_print_jobs(
          ...
          p_organization_id,v_print_job_id,p_location_id,v_receipt_id,p_purchase_order_id,v_input.item_id,v_input.pack_unit_id,
          v_input.good_pack_count,p_actor_id,v_lot_code,v_expires_on
        );
    ...
    if v_damaged>0 then
      ...
      insert into public.inventory_receipt_exceptions(
        organization_id,receipt_id,purchase_order_id,location_id,item_id,pack_unit_id,exception_type,pack_count,base_quantity,note,created_by
      ) values(
        p_organization_id,v_receipt_id,p_purchase_order_id,p_location_id,v_input.item_id,v_input.pack_unit_id,'damaged',
        v_input.damaged_pack_count,v_damaged,nullif(left(trim(coalesce(v_input.note,'')),500),''),p_actor_id
      );
    end if;
    if v_missing>0 then
      v_exception_count:=v_exception_count+1;
      insert into public.inventory_receipt_exceptions(
        ...
        p_organization_id,v_receipt_id,p_purchase_order_id,p_location_id,v_input.item_id,v_input.pack_unit_id,'missing',
        v_input.missing_pack_count,v_missing,nullif(left(trim(coalesce(v_input.note,'')),500),''),p_actor_id
      );
    end if;
  end loop;

  if v_good_line_count=0 and v_exception_count=0 then raise exception using errcode='22023',message='inventory_receipt_lines_invalid'; end if;
  if exists(
  ...
  v_result:=jsonb_build_object(
    'receiptId',v_receipt_id,'purchaseOrderId',p_purchase_order_id,'purchaseOrderStatus',v_status,
    'goodLineCount',v_good_line_count,'exceptionCount',v_exception_count,'printJobIds',v_print_jobs,'idempotent',false
  );
