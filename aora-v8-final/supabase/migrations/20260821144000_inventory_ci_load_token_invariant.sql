-- Verify the exact 100 CI session tokens rather than relying on a time-window
-- aggregate. This keeps the bootstrap invariant deterministic under concurrent CI.
...
begin
  ...
  from jsonb_array_elements_text(v_tokens) token(value)
  join public.app_sessions s
    on s.organization_id=p_organization_id
   and s.subject_id=p_subject_id
   and s.location_id=p_location_id
   and s.token_hash=digest(token.value,'sha256')
   and s.revoked_at is null;
  if v_verified<>p_count then raise exception 'inventory_ci_session_token_invariant'; end if;
