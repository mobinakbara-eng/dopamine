...
begin
  ...
  if p_rule_id like 'manual:%' and not exists(
    ...
    insert into public.task_rules(
      ...
      active,version,created_by,updated_by,created_at,updated_at,payload
    ) values(
      p_organization_id,p_rule_id,p_location_id,p_template_id,'manual','{}'::jsonb,
...
revoke all on function public.aora_create_scheduled_task_atomic(uuid,text,text,integer,text,text,text,timestamptz,timestamptz,date,boolean,text,jsonb) from public,anon,authenticated;
grant execute on function public.aora_create_scheduled_task_atomic(uuid,text,text,integer,text,text,text,timestamptz,timestamptz,date,boolean,text,jsonb) to service_role;
